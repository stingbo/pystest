    ...
    def get(self, type, content, index, config={}, element={}):
        ...
        if config and 'contents' in config.keys():
            ...
            if element:
                if index >= 0:
                    try:
                        result = element.find_elements(getType(type), content)
                        return result[index]
                    ...
                    return element.find_elements(getType(type), content)
                else:
                    return element.find_element(getType(type), content)
            else:
                if index >= 0:
                    try:
                    ...
                    return self.browser.find_elements(getType(type), content)
                else:
                    return self.browser.find_element(getType(type), content)
